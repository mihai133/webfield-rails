module Components::ButtonHelper
  def render_button(label = "", text: nil, variant: :default, as: :button, href: nil, method: nil, data: {}, **options, &block)
    ...
    button_classes = [button_classes, variant_classes, options[:class]].join(" ")

    text = label if label.present?
    text = capture(&block) if block
    render "components/ui/button", text: text, button_classes: button_classes, as: as, href: href, method: method, data: data, options: options
  end
end
